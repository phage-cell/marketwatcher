...
from . import GSA
from .addon import write_deals_to_addon
...
def send_to_logger_and_addon(*,
    ...
    if not any((item_deals, pet_deals)):
        if not GSA.settings['only_show_realms_with_deals']:
            realm = get_pretty_list_of_realms_on_connected_id(connected_realm_id=connected_realm_id,
                                                              region=GSA.settings['region'])
            logger.info(f"{', '.join(realm)} updated but zero deals found.\n")
        return
    ...
    send_to_logger(deals_items=item_deals, deals_pets=pet_deals, connected_realm_id=connected_realm_id)
...
def send_to_logger(*,
    ...
    for item in deals_items:
        c = GSA.shopping[connected_realm_id]['items'][item['item']['id']]
    ...
    for pet in deals_pets:
        c = GSA.shopping[connected_realm_id]['pets'][pet['item']['pet_species_id']]
        p = {
            "name": c['nickname'],
    ...
    if connected_realm_id in GSA.settings.get('connected_realm_nicknames', {}):
        realms = GSA.settings['connected_realm_nicknames'][connected_realm_id]
    else:
...
def send_to_addon(*,
    ...
    for item in deals_items:
        ...
        if 'unit_price' not in item:
            if item['gsa_item_level'] > 0:
                item_metadata['item_level'] = item['gsa_item_level']

            if item['gsa_item_suffix']:
                suffix_db = load_db(db='item_metadata.json')
                item_metadata['item_suffix'] = suffix_db[item['gsa_item_suffix']]

        deals["GSAData"][connected_realm_id].append(item_metadata)
    deals["GSAData"][connected_realm_id].reverse()

    write_deals_to_addon(addon_directory=addon_path, deals=deals)
